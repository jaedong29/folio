...
Folio는 토스증권 보유 주식과 바이낸스 현물 지갑을 모아보는 **연결 자산 화면**과, 사용자가 자산·거래를 직접 기록하는 화면을 제공합니다. 주문·출금 기능은 없습니다. 계좌 연결은 기본 비활성화이며, 현재 공식 API 기반 구현과 고정 응답 검증을 완료한 단계입니다. 실계좌 검증과 외부 서비스 배포는 아직 하지 않았습니다. [연결 범위·설정·검증 기록](docs/CONNECTED_PORTFOLIO.md)을 확인하세요.
...
2. 첫 화면은 연결 자산입니다. 연결 기능을 활성화한 서버에서 토스증권 또는 바이낸스 현물 연결 정보를 등록하면 자동으로 조회합니다. 연결된 평가금액·비중·보유 종목·갱신 상태를 확인합니다.
3. 직접 기록 화면에서는 가입 시 KRW·USD·USDT 투자 대기자금이 잔액 `0`으로 준비됩니다. 아래 수동 기록 흐름은 연결 자산 합계에 포함되지 않습니다.
4. 현금·은행·주식·암호화폐를 등록합니다.
5. 매수·매도는 같은 통화의 투자 대기자금과 함께 정산됩니다.
6. Dashboard에서 총자산, 오늘 손익, Position, Allocation, 환율, 최근 활동을 확인합니다.
7. 자산을 누르면 거래 내역, 평균 매수가, 실현손익, 최근 시장가격을 확인하고 오입력을 정정할 수 있습니다.
8. 오늘 손익을 누르면 Asset Analysis에서 기간별 Portfolio 흐름과 기여도를 확인합니다.
9. News에서 공용 공식자료를 보거나, 현재 등록한 자산과 관련된 자료만 필터링합니다.

| 영역 | 구현 내용 |
|---|---|
| Connected Portfolio | 토스증권 주식·바이낸스 현물 조회, AES-GCM 키 보관, 비동기 갱신, 연결 해제, 누락·오래된 평가값 표시 |
...
```text
com.assetdashboard
├── connection          제공처 계좌 연결·암호화·조회 작업·독립 스냅샷
├── global
...
| 타임아웃 | 제공자 요청당 2초 |
| Circuit Breaker | Yahoo/Binance/Upbit/GitHub/NIM별 독립 상태. 최근 10건 중 최소 5건 이후 실패율 50%면 30초 차단 |
| 재시도 | network·5xx GET만 지수 backoff. 가격 2회, GitHub 수집 3회(최초 포함). NIM POST는 중복 과금 방지를 위해 1회 |
...
차단기 상태 전환과 재시도는 민감정보 없이 로그에 남고 Resilience4j 지표는 인증이 필요한
`/actuator/metrics`에서 확인할 수 있습니다. 시도 횟수와 차단 기준은 `APP_RESILIENCE_*` 환경변수로 조정할 수
있습니다.
