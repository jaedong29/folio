...
| 엔비디아 (NVDA) | STOCK | 20개 @120×1,380 매수 + 수동 시세 219.22 / 환율 1,417 |
...
>
> 이 상태에서 시나리오 7(존재하지 않는 심볼 → 400)은 실패로 표시되는데, **그게 정상이다.** 외부가 죽으면 심볼 검증을 건너뛰고 등록을 허용하는 것이 PRD 4-2의 의도된 동작이다. 7번과 8번은 서로 배타적이라 두 모드로 나눠 실행한다.
